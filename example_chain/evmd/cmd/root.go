...
import (
	...
	cosmosevmcmdconfig "github.com/cosmos/evm/cmd/config"
	cosmosevmkeyring "github.com/cosmos/evm/crypto/keyring"
	"github.com/cosmos/evm/example_chain"
	cmdcfg "github.com/cosmos/evm/example_chain/evmd/config"
...
func initRootCmd(rootCmd *cobra.Command, osApp *example_chain.ExampleChain) {
	...
	cosmosevmserver.AddCommands(
		rootCmd,
		cosmosevmserver.NewDefaultStartOptions(newApp, example_chain.DefaultNodeHome),
		appExport,
		addModuleInitFlags,
	)
	...
	rootCmd.AddCommand(
		cosmosevmcmd.KeyCommands(example_chain.DefaultNodeHome, true),
	)
...
func newApp(
	...
	if chainID == "" {
		chainID, err = cosmosevmcmdconfig.GetChainIDFromHome(homeDir)
		if err != nil {
			panic(err)
		}
	}
...
func appExport(
	...
	if height != -1 {
		exampleApp = example_chain.NewExampleApp(logger, db, traceStore, false, appOpts, example_chain.EvmAppOptions)

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = example_chain.NewExampleApp(logger, db, traceStore, true, appOpts, example_chain.EvmAppOptions)
	}
